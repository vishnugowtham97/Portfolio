...
const ParticleBackground = ({ containerRef }) => {
  ...
  const getModeConfig = () => {
    const opts = JSON.parse(JSON.stringify(config.particles)); // deep clone
    const isSmallScreen = window.innerWidth < 768;

    if (isSmallScreen) {
      opts.number.value = 20;
      opts.line_linked.enable = false;
      opts.opacity.value = 0.15;
      opts.move.speed = 1;
      opts.size.value = 10;
    }

    return { opts, mode: isSmallScreen ? "minimal" : "network" };
  };
  ...
  useEffect(() => {
    ...
    class Particle {
      constructor() {
        this.x = Math.random() * canvas.width;
        this.y = Math.random() * canvas.height;
        this.radius = opts.size.random
          ? Math.random() * opts.size.value
          : opts.size.value;
        this.color = opts.color.value;
        this.opacity = opts.opacity.value;
        this.speedX = (Math.random() - 0.5) * opts.move.speed;
    ...
    const animate = () => {
      ...
      particles.forEach((p, i) => {
        ...
        if (mode === "network" && opts.line_linked.enable) {
          for (let j = i + 1; j < particles.length; j++) {
            const p2 = particles[j];
            const dist = Math.hypot(p.x - p2.x, p.y - p2.y);
            if (dist < opts.line_linked.distance) {
              drawLine(
                ...
                p2.x,
                p2.y,
                opts.line_linked.color,
                opts.line_linked.opacity,
                opts.line_linked.width
              );
            }
          }
        }
        ...
        if (
          ...
          const distMouse = Math.hypot(
            p.x - mousePosition.current.x,
            p.y - mousePosition.current.y
          );
          const grabDistance = 150;
          if (distMouse < grabDistance) {
            drawLine(
